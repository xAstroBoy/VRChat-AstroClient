...
using AstroClient.ClientUI.ActionMenu.AvatarParametersModule.Menu;
using AstroClient.Tools.PedestralImageEditor;
...
public class RenderCameraHijacker : AstroMonoBehaviour
{
    ...
    private bool HasAddedOurname { [HideFromIl2Cpp] get; [HideFromIl2Cpp] set; } = false;
    private bool HasAddedOurname2 { [HideFromIl2Cpp] get; [HideFromIl2Cpp] set; } = false;
    ...
    private bool isReadingCamera { [HideFromIl2Cpp]get; [HideFromIl2Cpp]set; } = false;
    internal override void UdonBehaviour_Event_SendCustomEvent(UdonBehaviour item, string EventName)
    {
        if (RenderCameraEvent != null && item.Equals(RenderCameraEvent.udonBehaviour))
        {
            if (renderCamera != null)
            {
                ...
                if (cam != null)
                {
                    var reader = new ReadRenderTextureAlpha(cam.ToBitMap(), OnReadingFinished);
                    if (reader != null)
                    {
                        reader.RUN();
                    }
    ...
    private void OnReadingFinished(string result)
    {
