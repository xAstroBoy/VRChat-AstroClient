namespace AstroClient.Startup.Hooks.PhotonHook.Tools.Replacers;
    ...
    internal static void ReplaceData(ref EventData __0, ref Hashtable ConvertedToNormalTable)
    {
        try
        {
            if (__0.Parameters[Photon_OnEventBytes.CustomData].GetIl2CppType().Equals(Il2CppType.Of<Il2CppSystem.Collections.Hashtable>()))
            {
                ...
                foreach (byte key in ConvertedToNormalTable.Keys) ModifiedEvent.Add(Il2CppConverter.Generate_Il2CPPObject(key), Serialization.FromManagedToIL2CPP<Object>(ConvertedToNormalTable[key]));
