...
using AstroClient.febucci.Core.Special_Tags;
...
using AstroClient.febucci.Effects.Scriptables;
...
namespace AstroClient.febucci
{
    ...
    public class TextAnimator : MonoBehaviour
    {
        ...
        public TextAnimator(IntPtr obj0) : base(obj0)
        {
        ...
        TAnimPlayerBase tAnimPlayer
        {
            get
            {
                ...
                if (_tAnimPlayer == null)
                {
                    Log.Error("Text Animator Player component is null on GameObject {gameObject.name}.");
                }
                return _tAnimPlayer;
            }
        }
	   ...
	   bool triggerAnimPlayerOnChange = false;
        ...
        AppearancesContainer appearancesContainer = new AppearancesContainer();
         ...
         string[] tags_fallbackBehaviors = new string[0];
         BehaviorDefaultValues behaviorValues = new BehaviorDefaultValues();

        //Global effect values
#pragma warning disable CS0649
         BuiltinBehaviorsDataScriptable scriptable_globalBehaviorsValues;
         BuiltinAppearancesDataScriptable scriptable_globalAppearancesValues;
#pragma warning restore CS0649
        ...
        public TMP_Text tmproText
        {
            get
            {
                ...
                if (_tmproText == null)
                {
                    Log.Error("TextAnimator: TMproText component is null.");
                }
                return _tmproText;
            }
        ...
        void BuildTagsDatabase()
        {
            ...
            AppearanceBase[] GetFallbackAppearancesFromTag(string[] tagsToConvert)
            {
                ...
                for (int i = 0; i < tagsToConvert.Length; i++)
                {
                    ...
                    {
                        Log.Error($"TextAnimator: Effect Tag '{tagsToConvert[i]}' is not recognized. {this.gameObject.name} will not be animated.");
                    }
                }
            ...
            for (int i = 0; i < tags_fallbackBehaviors.Length; i++)
            {
                ...
                {
                    Log.Error($"TextAnimator: Behavior Tag '{tags_fallbackBehaviors[i]}' is not recognized. {this.gameObject.name} will not be animated.");
                }
            }
