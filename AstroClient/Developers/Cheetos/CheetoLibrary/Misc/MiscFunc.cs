    ...
    public class MiscFunc
    {
        ...
        public static void CustomSerialize(bool Toggle)
        {
            try
            {
                if (Toggle)
                {
                    Capsule = UnityEngine.Object.Instantiate<GameObject>(GameInstances.CurrentUser.prop_VRCAvatarManager_0.prop_GameObject_0, null, true);
                    ...
                    Capsule.transform.position = GameInstances.CurrentUser.transform.position;
                    Capsule.transform.rotation = GameInstances.CurrentUser.transform.rotation;
                }
                if (!Toggle)
                {
        ...
        public static void PortalToInstance(APIUser user)
        {
            ...
            {
                GameInstances.VRCUiPopupManager.ShowAlert("[DayClient] Error", user.displayName + " Is a Private World");
                return;
            }

            if (user.id == APIUser.CurrentUser.id)
            {
                GameInstances.VRCUiPopupManager.ShowAlert("[DayClient] Error", "Cant Drop Portal on Yourself");
                return;
            }

            if (user.location == null || user.location == "")
            {
                GameInstances.VRCUiPopupManager.ShowAlert("[DayClient] Error",
        ...
        public static void EmojiRPC(int i)
        {
            try
            {
                ...
                Networking.RPC(0, GameInstances.CurrentUser.gameObject, "SpawnEmojiRPC", new Il2CppSystem.Object[]
                {
                    obj,
                });
            }
        ...
        public static void PlayEmoteRPC(int i)
        {
            try
            {
                ...
                var obj = x.BoxIl2CppObject();
                Networking.RPC(0, GameInstances.CurrentUser.gameObject, "PlayEmoteRPC", new Il2CppSystem.Object[]
                {
                    obj,
                });
            }
