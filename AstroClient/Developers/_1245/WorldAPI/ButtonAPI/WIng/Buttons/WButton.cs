...
internal class WButton : WingBtnControls
{
    internal WButton(WPage menu, string buttonName, Action listener, string toolTip, Sprite Icon = null, bool SubMenu = false, string Header = "") {
        ...
        TMProCompnt = Object.Instantiate(transform.Find("Text_Header"), transform.Find("Text_Header").parent).GetComponent<TextMeshProUGUI>();
        TMProCompnt.text = buttonName;
        TMProCompnt.fontSize = 28;
        Text = buttonName;
        TMProCompnt.transform.localPosition = new Vector3(-82, 0, 0);
        HeaderTMProCompnt = transform.Find("Text_Header").GetComponent<TextMeshProUGUI>();
