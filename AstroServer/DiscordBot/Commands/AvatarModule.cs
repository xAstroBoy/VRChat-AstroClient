namespace AstroServer.DiscordBot.Commands
{
    ...
    public class SearchModule : ModuleBase<SocketCommandContext>
    {
        ...
        {
            [Command("Name")]
            public async Task Name([Required] string query, int count = 1)
            {
                if (count > 10)
                {
                    count = 10;
            ...
            [Command("Author")]
            public async Task Author([Required] string query, int count = 1)
            {
                if (count > 10)
                {
                    count = 10;
                ...
                var avatars = await DB.Find<AvatarDataEntity>().ManyAsync(a => a.AuthorName.Equals(query));
                ...
                {
                    await ReplyAsync($"No avatars found by: {query}");
                }
            }
        }
